...
def shift_is_valid(shift, langua):
    if shift.isdigit():
        if langua == 'russian' and 1 <= int(shift) <= 32:
            lbl_incorrect_format.configure(pady='-50', fg='LightSteelBlue3')
            return True
        elif langua == 'english' and 1 <= int(shift) <= 26:
            lbl_incorrect_format.configure(pady='-50', fg='LightSteelBlue3')
            return True
        else:
...
def language_is_valid(langu, text):
    if langu == 'russian' and len(text) != 0:
        for l in range(len(text)):
            if text[l] in russian_alphabet or text[l] in russian_alphabet_upper:
                return True
    elif langu == 'english' and len(text) != 0:
        for l in range(len(text)):
...
def validation(proc, lang, shift, text):
    ...
    if proc == 0:
        ...
        lbl_processNotChosen = Label(window, text='Should not be empty', fg='LightSteelBlue3', width='17',
                                     bg='LightSteelBlue3')
        lbl_processNotChosen.place(x=105, y=40)
    ...
    if lang == 0:
        ...
        lbl_langNotChosen = Label(window, text='Should not be empty', fg='LightSteelBlue3', width='17',
                                  bg='LightSteelBlue3')
        lbl_langNotChosen.place(x=115, y=93)
    ...
    if len(text) == 0:
        ...
        lbl_incorrect_language.place(x=350, y=227)
    else:
...
def generate():
    ...
    if validation(proc, lang, shift, text):
        ...
        if language_is_valid(language, text):
            ...
            lbl_incorrect_language.configure(text='Some letters are not in a language you chosen', fg='red', width='35')
            lbl_incorrect_language.place(x=350, y=227)
...
lbl_incorrect_language = Label(window, width='19', bg='LightSteelBlue3')
